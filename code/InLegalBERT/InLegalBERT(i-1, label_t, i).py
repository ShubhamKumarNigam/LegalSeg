...
def process_data_chunk(chunk, tokenizer):
    ...
    for i in range(len(chunk)):
        prev_label = 'Facts'
        if i>0 and chunk['label'].iloc[i-1] != 'Facts':
            prev_label = chunk['label'].iloc[i-1]
        prev_text = chunk['text'].iloc[i-1] if i > 0 else ""
        curr_text = chunk['text'].iloc[i]
        
        combined_text = f"{prev_text} [SEP] {prev_label} [SEP] {curr_text}"
...
def generate_np_files_for_training(dataf, tokenizer):
    ...
    for i in progressbar.progressbar(range(len(dataf['text']))):
        # Get current, previous, and next texts
        prev_label = 'Facts'
        if i>0 and dataf['label'].iloc[i-1] != 'Facts':
            prev_label = dataf['label'].iloc[i-1]
        curr_text = dataf['text'].iloc[i]
        prev_text = dataf['text'].iloc[i-1] if i > 0 else ""
...
tokenizer = AutoTokenizer.from_pretrained("/home/shubham/RR/req/") #("law-ai/InLegalBERT")
...
print("################################")
model = BertForSequenceClassification.from_pretrained("/home/shubham/RR/req/", num_labels=7)
