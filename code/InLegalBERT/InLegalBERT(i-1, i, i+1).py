...
validation_set = pd.read_csv('../../data/InLegalBERT/val.csv')
...
def process_data_chunk(chunk, tokenizer):
    ...
    for i in range(len(chunk)):
        ...
        next_text = chunk['text'].iloc[i+1] if i < len(chunk)-1 else ""
        
        combined_text = f"{prev_text} [SEP] {curr_text} [SEP] {next_text}"
...
def generate_np_files_for_training(dataf, tokenizer):
    ...
    for i in progressbar.progressbar(range(len(dataf['text']))):
        ...
        next_text = dataf['text'].iloc[i+1] if i < len(dataf['text'])-1 else ""
...
def input_id_maker(dataf, tokenizer):
    ...
    for i in range(len(dataf['text'])):
        prev_text = dataf['text'].iloc[i-1] if i > 0 else ""
        curr_text = dataf['text'].iloc[i]
        next_text = dataf['text'].iloc[i+1] if i < len(dataf)-1 else ""
