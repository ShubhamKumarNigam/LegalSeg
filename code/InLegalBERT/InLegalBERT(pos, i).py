...
def process_data_chunk(chunk, tokenizer):
    ...
    for index, row in chunk.iterrows():
        text = row['text']
        length = len(train_set[train_set['index'] == row['index']])
        a = index - train_set[train_set['index'] == row['index']].index[0] + 1
        pos = a / length
        if pos > 1:
            raise ValueError(f"Position value `pos` should be between 0 and 1, but got: {pos}")
        text = f"{pos} [SEP] {text}"
...
#         print(len(b_input_ids), len(b_input_mask), len(b_labels))
    ...
    out_path = "/home/shubham/RR/Inlegal/output"
    output_dir = out_path + '/output_pos,i,emb/'
