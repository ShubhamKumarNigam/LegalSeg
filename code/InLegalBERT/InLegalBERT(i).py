...
def metrics_calculator(preds, test_labels, num_classes=7):
    ...
    total_TN = sum([sum([cm[j][k] for j in range(num_classes) for k in range(num_classes) if j != i and k != i]) for i in range(num_classes)])

    micro_precision = total_TP / (total_TP + total_FP) if (total_TP + total_FP) != 0 else 0
    micro_recall = total_TP / (total_TP + total_FN) if (total_TP + total_FN) != 0 else 0
    micro_f1 = (2 * micro_precision * micro_recall) / (micro_precision + micro_recall) if (micro_precision + micro_recall) != 0 else 0

    mcc = (total_TP * total_TN - total_FP * total_FN) / (((total_TP + total_FP) * (total_TP + total_FN) * (total_TN + total_FP) * (total_TN + total_FN)) ** 0.5) if ((total_TP + total_FP) * (total_TP + total_FN) * (total_TN + total_FP) * (total_TN + total_FN)) != 0 else 0
...
print("macro_precision", "\t", "macro_recall", "\t\t", "macro_f1", "\t\t", "accuracy", "\t\t", "MCC")
print(metrics['macro_precision'], "\t", metrics['macro_recall'], "\t", metrics['macro_f1'], "\t", flat_accuracy(predictions, true_labels), "\t", metrics['mcc'])
