# -*- coding: utf-8 -*-
"""Dummy_role_aware.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Fz_P7xS8Oz_pS1J7uoAEmbIYVmCZqIbA
"""
...
def evaluate_model(model, dataloader, epoch=None, writer=None, phase='Validation'):
    ...
    all_labels = []
    total_loss = 0
    criterion = nn.CrossEntropyLoss(weight=criterion_weight)  # Use same weights as during training
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    def preprocess_df(df):
       # df = df.dropna(subset=['Text', 'Label'])  # Drop rows with missing values
        ...
        df.loc[:, 'Label'] = df['Label'].map(label_to_int)
    ...
    # (Optional) Create a weighted sampler
    ...
    # Create dataloaders
    train_dataloader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True)  # Use sampler if needed
    ...
    criterion_weight = class_weights
    ...
    # Load the best model
    model.load_state_dict(torch.load('../../saved_models/Role-Aware/Role-Aware_CI/'))
